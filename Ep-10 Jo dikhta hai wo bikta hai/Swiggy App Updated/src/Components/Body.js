...
const Body = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            // ---- FIX: find restaurants dynamically (no hard-coded [4]) ----
            const cards = json?.data?.cards || [];
            let restaurants = [];
            for (const c of cards) {
                const a = c?.card?.card?.gridElements?.infoWithStyle?.restaurants;
                if (Array.isArray(a)) { restaurants = a; break; }
                const b = c?.card?.gridElements?.infoWithStyle?.restaurants;
                if (Array.isArray(b)) { restaurants = b; break; }
                const grouped = c?.groupedCard?.cardGroupMap?.REGULAR?.cards;
                if (Array.isArray(grouped)) {
                    for (const g of grouped) {
                        const r = g?.card?.card?.gridElements?.infoWithStyle?.restaurants;
                        if (Array.isArray(r)) { restaurants = r; break; }
                    }
                        ...
                        <button className="m-4 px-4 py-2 bg-gray-200 flex items-center rounded-lg"
                            onClick={() => {
                                ...
                                const filteredList = listOfRestaurants.filter(
                                    (res) => Number(res.info.avgRating ?? res.info.avgRatingString ?? 0) > 4.1
                                );
